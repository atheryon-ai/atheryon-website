...
- `npx playwright test --project=chromium` — the suite CI runs (81 tests, ~1 min). There is no `npm test` script; `playwright.config.ts` defines a single `chromium` project and starts `next dev` itself via `webServer`.
