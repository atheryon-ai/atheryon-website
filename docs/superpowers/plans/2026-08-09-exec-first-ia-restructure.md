...
Revision: 5 (2026-08-09) — arms model: M&A and Capital Markets as the two business areas over a DATA · TRANSFORMATION · AI underpinning; Terry-authored root hero stack; /ma reprieved as the M&A arm route; /capital-markets replaces /technology
...
|---|---|---|
| Hero + firm intro (7 paras) | `/` | Replace with the Appendix B root hero stack verbatim (hero, subheading, supporting copy); then the proof strip: RAMS $21.4bn integration, >$1bn separation programs, four FS sectors, four jurisdictions (all-transaction for now — add one technology proof point when §8 TODO 6 lands). Two CTAs: primary → /contact, secondary → /ma. Audience line and remaining intro paragraphs move to /about. |
...
| (no source — Appendix B) | `/` arms section | Two equal cards over the underpinning strip `DATA · TRANSFORMATION · AI`: M&A first → /ma; Capital Markets second → /capital-markets. Phase 1: hide the Capital Markets card behind an `isPending` guard until phase 2 — no dead links. |
