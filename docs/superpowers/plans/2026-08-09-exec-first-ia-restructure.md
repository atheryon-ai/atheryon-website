...
Revision: 7 (2026-08-09) — content-separation rule: capital markets and transaction content do not mix; /capital-markets goes standalone-only with `{{...}}` blanks for Terry where content doesn't exist yet; the three transaction workflows relocate to /ma. (Rev 6: poster-as-homepage. Rev 5: arms model; /ma reprieved; /capital-markets replaces /technology.)
...
- Content separation (rev 7, Terry 2026-08-09: "do not mix capital markets with transaction"): `/capital-markets` carries only standalone capital-markets material — systems and platform delivery, market data environments and reference data, enterprise architecture, data platforms, AI-enabled delivery. Its transaction role appears as exactly ONE cross-link line ("Inside a transaction, this arm runs the Technology, Data & Migration Readiness service line →" → /ma). The three transaction workflows (Pre-Sign Execution Review, Separation/Integration Planning, TSA Tracking & Reduction) relocate to /ma as a collapsed detail under service line 04. Where standalone CM content does not exist yet (cases, delivery examples, proof point), render nothing and mark the site.ts slots with `{{...}}` placeholders behind `isPending` guards — never fill the gap with transaction material.
...
| Services (4 lines) | `/ma` | M&A arm page (transformation and operating-model scope included); hero "Making Transactions Executable". Rev 7: the three transaction workflows (Pre-Sign Execution Review, Separation/Integration Planning, TSA Tracking & Reduction) live here as a collapsed/secondary detail under service line 04. TSA items currently appear in all four lines; dedupe so each line owns distinct TSA scope (readiness: TSA strategy; strategy: TSA design/exit planning; execution: TSA establishment/exit management; technology: TSA-driving data dependencies). Add an engagement section at page end: embedded senior specialists alongside the client team, pre-sign through operational independence. No stated durations (decided 2026-08-09: drop "6–18 months"). |
| Representative Experience (5 cases) | `/experience` | Normalise schema: every case gets Context / Role / Outcome. RAMS first. Others keep anonymised client descriptors as written. Rev 7: a Capital Markets section renders after the five transaction cases, hidden behind `isPending` until `{{CM_CASE_1}}` / `{{CM_CASE_2}}` are filled — never mixed into the transaction list. |
