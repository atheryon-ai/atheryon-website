...
export function Footer({ mode = 'cm' }: { mode?: Mode }) {
  ...
  // href (URL not provided) renders no LinkedIn link at all.
  const linkedinHref =
    mode === 'ma'
      ? site.footer.maLinkedin
      : 'https://www.linkedin.com/in/terencetsakiris/'

  // /ma/* keeps its flat practice link set until phase 3 retires the group.
  if (mode === 'ma') {
    const navLinks = [...site.footer.maLinks, ...legalLinks]
    return (
